...
{`'use client'; // Required for navigation hooks
...
export default function NavigationDemo() {
  const router = useRouter();
  const pathname = usePathname(); // "/navigation-demo"
  const searchParams = useSearchParams(); // URLSearchParams object
  
  const handleNavigation = () => {
    router.push('/posts/123'); // Programmatic navigation
    router.back(); // Go back
  ...
  return (
    <div>
      ...
      <p>Query params: {searchParams.get('tab')}</p>
